...
const Home = () => {
  ...
  const downloadExcel = async () => {
    try {
      ...
      const res = await fetch(`/api//user?page=download`);
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <BaseLayout>
      <div
        className={`${styles.headertext} font-spartan flex justify-center font-bold text-[#ffcc00] p-10 text-3xl`}
      >
        Registered Users
      </div>
     ...
     {loading?<div className="flex justify-center bg-[#4F3F00] rounded-lg items-center h-[70vh]">
          <span class={`${styles.loader}`}></span>
         </div>:    <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
        {user ? (
          <div>
          {user.map((user,index) => (
            <div key={index}>
