...
const ViewallApplications = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const filteredJobApplications = data.filter(
          (jobApplication) => jobApplication.status !== "deleted" && jobApplication.ApprovalStatus === "rejected"
        );
  ...
  const AcceptApplication = async (applicationId,email) => {
   router.push(`/dashboard/sendmail?applicationId=${applicationId}&email=${email}`);
  };
